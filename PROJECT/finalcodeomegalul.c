...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE END WHILE */
	     ...
	     char buffer[40];
	     //UART_SendString("Hello from Transmitter\n");
	     UART_SendString("Hello from Transmitter\n");
	     sprintf(buffer, "Temp f: %f\n",tFahrenheit);
	     UART_SendString(buffer);
	     sprintf(buffer, "Temp C: %f\n",tCelsius);
	     UART_SendString(buffer);
	     sprintf(buffer, "RH: %f\n",RH);
	     UART_SendString(buffer);
	     sprintf(buffer, "Current Input: %i\n",rxData);
	     UART_SendString(buffer);
	     ...
	     //UART_SendString("Temperature C: %f", tFahrenheit);
	     //UART_SendString("Relative Humidity: %f" , RH);
//	     /UART_SendString("User input (LED): ")
    /* USER CODE BEGIN 3 */
	   }
  /* USER CODE END 3 */
}
